...
export const processBotProfileUpdate = async (history: HistoryItem[]) => {
  ...
  const completion = await ai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Первоначальный профиль: ${current}. История сообщений: ${
          history.slice(-50).map((i) =>
            `role: ${i.role}; ${i.name}; ${i.content}`
          ).join("\n")
        }`,
      },
    ],
